...
class _SearchFromState extends State<SearchFrom> {
                        ...
                        children: [
                          ...
                          InputContainer(
                              ...
                              child: SearchableDropdown(
                                items: searchModel.organizationsDescriptions
                                    .map((item) {
                                  return DropdownMenuItem<
                                          OrganizationsDescription>(
                                      child: Text(
                                          "${item.organizationDescription} - (${item.code})"),
                                      value: item);
                                }).toList(),
                                isExpanded: true,
                          ...
                          InputContainer(
                              ...
                              child: SearchableDropdown(
                                items:
                                    searchModel.locationDescriptions.map((item) {
                                  return DropdownMenuItem<LocationDescription>(
                                      child: Text(
                                          "${item.description} - (${item.provinceCode})"),
                                      value: item);
                                }).toList(),
                                isExpanded: true,
                              ...
                              child: SearchableDropdown(
                                items: searchModel.budgetTypes.map((item) {
                                  return DropdownMenuItem<BudgetType>(
                                      child: Text(
                                          "${item.description} - (${int.parse(item.code)})"),
                                      value: item);
                                }).toList(),
                                isExpanded: true,
  ...
  void _submitForm() async {
    if (_m16Controller.text.isEmpty ||
      ...
      Map<String, dynamic> successInfo = await _appState.getM16Details({
        ...
        "budgetType": _selectedBudgetType.code.toString(),
        "sequenceNo": "0",
        "token": "token"
      });
      if (successInfo['success'] && successInfo['stageCount'] > 0) {
        ...
        Navigator.of(context).push(MaterialPageRoute(
            builder: (BuildContext context) => M16DetailsScreen()));
      } else if (successInfo['stageCount'] == 0) {
        LoadingDialog.hideLoadingDialog(context);
        MsgDialog.showMsgDialog(context, 'Alert!', 'M16 Not Found!');
      } else {
        LoadingDialog.hideLoadingDialog(context);
        MsgDialog.showMsgDialog(context, 'Alert!', 'Something went wrong!');
      }
    }
  }
}
